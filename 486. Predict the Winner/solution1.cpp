...
class Solution {
 ...
 private:
  // 递归地计算两个玩家的分数差
  int score(const vector<int>& nums, int begin, int end, int turn) {
    if (begin == end) {
      return nums[begin] * turn;
    }

    auto scoreBegin = nums[begin] * turn + score(nums, begin + 1, end, -turn);
    auto scoreEnd = nums[end] * turn + score(nums, begin, end - 1, -turn);

    return max(scoreBegin * turn, scoreEnd * turn) * turn;
  }
};
// @lc code=end
