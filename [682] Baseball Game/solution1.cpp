 ...
 public:
  int calPoints(vector<string>& ops) {
    ...
    for (const auto& op : ops) {
      if (op == "+") {
        const auto num1 = points[points.size() - 1];
        const auto num2 = points[points.size() - 2];
        points.emplace_back(num1 + num2);
      } else if (op == "D") {
        points.emplace_back(points.back() * 2);
      } else if (op == "C") {
        points.pop_back();
      } else {
        points.emplace_back(stoi(op));
      }
    }
    return accumulate(points.cbegin(), points.cend(), 0);
  }
};
// @lc code=end
