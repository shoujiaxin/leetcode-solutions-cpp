 ...
 public:
  vector<int> exclusiveTime(int n, vector<string>& logs) {
    ...
    for (const auto& log : logs) {
      const auto items = split(log, ':');
      const auto id = std::stoi(items[0]);
      const auto& event = items[1];
      const auto time = std::stoi(items[2]);

      if (event == "start") {
        if (!stk.empty()) {
          result[stk.top().first] += time - stk.top().second;
          stk.top().second = time;
        }

        stk.emplace(id, time);
      } else if (event == "end") {
        const auto [i, t] = stk.top();
        stk.pop();
